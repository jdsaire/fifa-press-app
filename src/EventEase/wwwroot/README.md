...
- **`lib/`** — **third-party code, not written for this project.** It holds Bootstrap ([Glossary.md](../../../learning-mode/Glossary.md#bootstrap)), a pre-built CSS toolkit this app uses for its buttons, cards, and layout styling. Nothing in `lib/` was authored as part of building EventEase, and it isn't documented or commented on any further than this paragraph.

This folder is separate from everything else in `src/EventEase/` because its contents aren't compiled or interpreted by .NET at all — they're just files, copied into the finished app as-is, the same way they'd be handled on any plain website.
