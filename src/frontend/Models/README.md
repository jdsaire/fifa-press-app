...
- **`LocalizedText.cs`** — one piece of authored content in all three languages at once, used for every free-text field on a `Change`. Not a translation performed at render time — it's written three times, deliberately, the way a real accreditation system would author the same notice in three languages at the moment it's sent rather than running it through a translator afterward.
