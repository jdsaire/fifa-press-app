...
- **`i18n/`** — `en.json`, `es.json`, `pt.json`: every piece of interface text this app renders, plus each language's month names and date patterns, one file per language, all fetched once when the app starts. See [`src/interop/README.md`](../../interop/README.md) for how this fits together with the two files below.
- **`js/`** — `theme.js` and `locale.js`, the two small pieces of interop this app needs: reading and writing one `localStorage` key each, and setting one attribute on the page (`data-theme`, `lang`). Both are **compiled output** — written in TypeScript in `src/interop/`, outside this project, and checked in here so nothing about running or deploying this app needs a JavaScript toolchain. Neither file holds a colour or a translated word; those live in `css/app.css` and `i18n/` respectively.
